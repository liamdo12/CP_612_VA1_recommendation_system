"""
SVD Matrix Factorization (Temporal Split - WITH Cold-Start)
...
print("="*60)
print("SVD MATRIX FACTORIZATION (TEMPORAL SPLIT - WITH COLD-START)")
print("="*60)
...
N_FACTORS = 30  # Number of latent factors for SVD
BATCH_SIZE = 5000  # Batch predictions to avoid full matrix reconstruction
...
# ============================================================================
print("\n" + "="*60)
print("MEAN-CENTERING USER-ITEM MATRIX")
print("="*60)
...
for user_idx in tqdm(range(user_item_matrix.shape[0]), desc="User means"):
...
print(f" User means computed (avg: {user_means.mean():.3f})")
...
for user_idx in tqdm(range(user_item_matrix.shape[0]), desc="Centering users"):
    # Get row data (only non-zero elements)
...
print(f"Computing SVD with {N_FACTORS} latent factors...")
print("This may take 5-10 minutes...")

start_time = time.time()

# Perform SVD
# svds returns: U (users x factors), sigma (factors,), Vt (factors x items)
...
print(f"  Sigma (factors): {sigma.shape}")
print(f"  Vt (factors � items): {Vt.shape}")

# Store SVD components for on-demand prediction (MEMORY EFFICIENT!)
# DO NOT reconstruct full matrix - would use ~30GB RAM!
print("\n✅ SVD components stored (U, sigma, Vt)")
print(f"  Memory usage: {(U.nbytes + sigma.nbytes + Vt.nbytes) / (1024**2):.2f} MB")
print(f"  vs Full matrix would be: {(U.shape[0] * Vt.shape[1] * 8) / (1024**3):.2f} GB")
...
def predict_rating(user_idx, movie_idx):
    """
    Predict rating using SVD model with ON-DEMAND computation.

    Memory-efficient implementation that computes predictions WITHOUT
    reconstructing the full matrix (~30GB saved!).
    ...
    # Compute prediction on-demand: (U[user] * sigma) dot Vt[:, movie]
    # This is mathematically equivalent to: (U @ diag(sigma) @ Vt)[user, movie]
    # But only computes ONE element instead of entire 30GB matrix!
    user_factors = U[user_idx, :]  # Shape: (n_factors,)
    item_factors = Vt[:, movie_idx]  # Shape: (n_factors,)

    # Weighted dot product
    centered_prediction = np.dot(user_factors * sigma, item_factors)

    # Add user mean back (de-center)
    prediction = centered_prediction + user_means[user_idx]
...
print("This is very fast with SVD (pre-computed matrix)...")
...
print(f"  SVD predictions: {len(predictions) - global_mean_predictions:,}")
...
print("\n" + "="*60)
print("SVD RESULTS (TEMPORAL SPLIT - WITH COLD-START)")
print("="*60)
...
print(f"\nTiming:")
print(f"  Training (SVD): {training_time/60:.2f} minutes")
...
print(f"  SVD predictions: {len(predictions) - global_mean_predictions:,} ({(len(predictions) - global_mean_predictions)/len(predictions)*100:.2f}%)")
...
results = {
    ...
    'svd_predictions': len(predictions) - global_mean_predictions,
    'cold_start_predictions': global_mean_predictions,
    'correlation': correlation
}

results_df = pd.DataFrame([results])
output_path = os.path.join(project_root, 'datasets/output/model_implementations/svd_with_cold_start.csv')
